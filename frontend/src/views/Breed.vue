    ...
    <el-card shadow="hover" class="main-card">
      ...
      <div class="action-bar">
        ...
        <div class="right-actions">
          ...
          <el-button type="success" icon="el-icon-download" size="medium" @click="exportVisible = true">导出数据
          </el-button>
        </div>
      </div>
      ...
      <!-- 新增/编辑 弹出栏 -->
      <el-dialog
          :title="addDialogFlag ? '添加品种' : '修改品种信息'" :visible="formVisibleAdd || formVisibleEdit" @close="() => { formVisibleAdd = false; formVisibleEdit = false; }"
        ...
        <el-form :model="dialogForm" :rules="formRules" ref="dialogForm" label-width="100px">
          ...
          <div class="dialog-footer">
            <el-button @click="formVisibleAdd = false; formVisibleEdit = false;">取消</el-button>
            <el-button type="primary" :disabled="isSubmit"
                       @click="addDialogFlag ? handleAdd('dialogForm') : handleEditSave('dialogForm')">确定
            </el-button>
          </div>
        </el-form>
      </el-dialog>
  ...
  methods: {
    ...
    handleDelete(index, row) {
      ...
      this.$confirm('此操作将删除选中数据, 是否继续?', '提示', {
        ...
        Request.delete("/breed/deleteBatch?ids=" + delId).then(response => {
          if (response.code === '0') {
            this.$message({
              type: 'success',
              message: '删除成功!'
    ...
    handleAdd(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          Request.post("/breed/save", this.dialogForm).then(response => {
            if (response.code == 0) {
              this.$message({
                showClose: true,
                message: '添加成功',
    ...
    handleEditSave(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          Request.put("/breed/update", this.dialogForm).then(response => {
            if (response.code == 0) {
              this.$message({
                showClose: true,
                message: '更新成功',
