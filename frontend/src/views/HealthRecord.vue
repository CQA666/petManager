        ...
        <el-card shadow="always">
            ...
            <el-form ref="searchForm" :inline="true" :model="listQuery" label-width="90px" class="search-form">
                ...
                <el-form-item label="记录日期">
                    <el-date-picker v-model="listQuery.recordDate" type="date" placeholder="记录日期" value-format="yyyy-MM-dd"/>
                </el-form-item>
            ...
            <el-dialog title="添加宠物健康记录" :visible.sync="formVisibleAdd" width="30%" class="dialog-form" :modal="false">
                ...
                <el-form ref="dialogForm" :model="dialogForm" :rules="formRules" label-width="120px">
                    ...
                    <el-form-item prop="recordDate" label="记录日期">
                        <el-date-picker v-model="dialogForm.recordDate" type="date" placeholder="记录日期"  value-format="yyyy-MM-dd"/>
                    </el-form-item>
                    ...
                    <el-form-item prop="vaccinationDate" label="疫苗接种日期">
                        <el-date-picker v-model="dialogForm.vaccinationDate" type="date" placeholder="疫苗接种日期" value-format="yyyy-MM-dd"/>
                    </el-form-item>
    ...
    methods: {
        ...
        handleAdd(formName) {
            this.$refs[formName].validate(valid => {
                if (valid) {
                    Request.post("/health-record/save", this.dialogForm).then(response => {
                        if (response.code == 0) {
                            this.$message({
                                showClose: true,
                                message: '宠物健康记录添加成功',
        ...
        handleDelete(index, row) {
            this.$confirm('此操作将删除选中数据, 是否继续?', '提示', {
                ...
                Request.delete("/health-record/deleteBatch?ids=" + row.id).then(response => {
                    if (response.code == 0) {
                        this.$message({
                            type: 'success',
                            message: '删除成功!'
        ...
        handleEditSave(formName) {
            this.$refs[formName].validate(valid => {
                if (valid) {
                    Request.put("/health-record/update",this.dialogForm).then(response => {
                        if (response.code == 0) {
                            this.$message({
                                showClose: true,
                                message: '更新成功',
