    ...
    methods: {
        ...
        handlePay(orderNo){
            ...
            this.$confirm('请选择支付方式', '支付选择', {
                ...
                // 支付宝支付
                const payWindow = window.open('http://localhost:8889/alipay/pay?orderNo=' + orderNo);
        ...
        checkPaymentStatus(orderNo) {
            ...
            const checkInterval = setInterval(() => {
                // 直接查询支付状态，不再检查窗口是否关闭
                Request.get('/alipay/query?orderNo=' + orderNo).then(response => {
                    if (response.code === '0') {
                        // 支付成功
                        ...
                        // 不立即结束轮询，可能是查询暂时失败
                        console.log('查询支付状态:', response.msg);
                    }
                }).catch(err => {
                    console.error(err);
                    // 查询出错，但不停止轮询
            ...
            setTimeout(() => {
                if (checkInterval) {
                    ...
                    this.$message({
                        type: 'info',
                        message: '支付状态查询超时，请刷新页面查看最新订单状态',
                        showClose: true,
                        duration: 5000
                    });
                }
        ...
        handleDelete(index, row) {
            this.$confirm('此操作将删除选中数据, 是否继续?', '提示', {
                ...
                Request.delete("/orders/deleteBatch?ids=" + row.id).then(response => {
                    if (response.code == '0') {
                        this.$message({
                            type: 'success',
                            message: '删除成功!'
        ...
        handleEditSave(order, updateMsg) {
            Request.put("/orders/update",order).then(response => {
                if (response.code == 0) {
                    this.$message({
                        showClose: true,
                        message: updateMsg,
