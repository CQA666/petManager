<template>
    <div class="user-manager">
        <el-card shadow="hover" class="main-card">
            ...
            <div class="action-bar">
                ...
                <div class="right-actions">
                    <el-button type="primary" icon="el-icon-plus" size="medium" @click="handleCreate">添加用户</el-button>
            ...
            <el-form ref="searchForm" :inline="true" :model="listQuery" class="search-form">
                <el-form-item label="用户名">
                    <el-input v-model="listQuery.username" placeholder="请输入用户名" prefix-icon="el-icon-user" />
                </el-form-item>
                <el-form-item label="姓名">
                    <el-input v-model="listQuery.name" placeholder="请输入姓名" prefix-icon="el-icon-s-custom" />
                </el-form-item>
                <el-form-item v-show="userInfo.role == 'SUPER_ADMIN'" label="角色">
                    <el-select v-model="listQuery.role" placeholder="请选择角色" style="width: 120px">
                        <el-option label="管理员" value="ADMIN"></el-option>
                        <el-option label="普通用户" value="USER"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="性别">
                    <el-select v-model="listQuery.sex" placeholder="请选择性别" style="width: 120px">
            ...
            <el-table ref="multipleTable" v-loading="listLoading" :data="tableData" @selection-change="handleSelectionChange"
                class="user-table" border stripe>
                ...
                <el-table-column v-if="userInfo.role == 'SUPER_ADMIN'" prop="role" label="角色" width="100" align="center">
                    <template slot-scope="scope">
                        <el-tag :type="scope.row.role == 'ADMIN' ? 'warning' : 'success'" size="small">
        ...
        <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" :modal="false"
            width="500px" class="user-dialog" :close-on-click-modal="false">
            <el-form ref="dialogForm" :model="dialogForm" :rules="formRules" label-width="80px" class="dialog-form">
                <div v-if="isEdit" class="avatar-upload">
                        <el-upload class="avatar-uploader" :action="uploadUrl" :show-file-list="false"
                            :on-success="handleAvatarSuccess" :before-upload="beforeAvatarUpload">
                        <img v-if="dialogForm.avatar" :src="imgBaseUrl + '/' + dialogForm.avatar" class="avatar" />
                    ...
                    <el-form-item v-if="userInfo.role == 'SUPER_ADMIN'" label="角色" prop="role">
            ...
            <div slot="footer" class="dialog-footer">
                ...
                <el-button type="primary" :disabled="isSubmit" @click="handleSubmit">确 定</el-button>
                    </div>
            </el-dialog>
    ...
    methods: {
        ...
        handleDelete(index, row) {
            this.$confirm('此操作将删除选中数据, 是否继续?', '提示', {
                ...
                Request.delete("/user/deleteBatch?ids=" + row.id).then(response => {
                    if (response.code == 0) {
                        this.$message({
                            type: 'success',
                            message: '删除成功!'
        ...
        batchDelete() {
            if (this.multipleSelection.length < 1) {
                ...
                Request.delete(`/user/deleteBatch?ids=${this.multipleSelection.join(',')}`).then(res => {
                    if (res.code === '0') {
                        this.$message({
                            showClose: true,
                            message: '批量删除成功',
        ...
        handleEditSave(formName) {
            this.$refs[formName].validate(valid => {
                if (valid) {
                    Request.put("/user/" + this.dialogForm.id, this.dialogForm).then(response => {
                        if (response.code == 0) {
                            this.$message({
                                showClose: true,
                                message: '更新成功',
        ...
        exportTable(type) {
            if (this.tableData.length) {
                const params = {
                    header: ['编号', '用户名', '姓名', '角色', '性别', '电话', '邮箱'],
                    key: ['id', 'username', 'name', 'role', 'sex', 'phone', 'email'],
                    data: this.tableData,
                    autoWidth: true,
                    fileName: '用户数据表',
                    bookType: type
