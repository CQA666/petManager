    ...
    methods: {
        ...
        handleAdd(formName) {
            this.$refs[formName].validate(valid => {
                if (valid) {
                    this.isSubmit = true;
                    Request.post("/room/save", this.dialogForm).then(response => {
                        if (response.code == 0) {
                            this.$message({
                                showClose: true,
                                message: '房间添加成功',
        ...
        handleDelete(index, row) {
            this.$confirm('此操作将删除选中数据, 是否继续?', '提示', {
                ...
                Request.delete("/room/deleteBatch?ids=" + row.id).then(response => {
                    if (response.code == 0) {
                        this.$message({
                            type: 'success',
                            message: '删除成功!'
        ...
        handleEditSave(formName) {
            this.$refs[formName].validate(valid => {
                if (valid) {
                    this.isSubmit = true;
                    Request.put("/room/update", this.dialogForm).then(response => {
                        if (response.code == 0) {
                            this.$message({
                                showClose: true,
                                message: '更新成功',
