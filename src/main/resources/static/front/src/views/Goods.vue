        ...
        <el-card shadow="hover" class="main-card">
            ...
            <div class="action-bar">
                ...
                <div class="right-actions">
                    <el-button type="primary" icon="el-icon-plus" size="medium" @click="handleCreate">添加商品</el-button>
            ...
            <el-form ref="searchForm" :inline="true" :model="listQuery" label-width="90px" class="search-form">
                <el-form-item label="商品名称">
                    <el-input v-model="listQuery.name" placeholder="请输入商品名称" prefix-icon="el-icon-goods" />
                </el-form-item>
                <el-form-item>
            ...
            <el-table ref="multipleTable" v-loading="listLoading" :data="tableData"
                @selection-change="handleSelectionChange" class="goods-table" border stripe>
                <el-table-column type="selection" width="60" align="center" />
                <el-table-column prop="id" label="编号" width="80" sortable align="center" />
                ...
                <el-table-column prop="img" label="商品图片" width="100" align="center">
                    <template slot-scope="scope">
                        <el-image :src="imgBaseUrl + '/' + scope.row.img" alt="商品图片"
                            :preview-src-list="[imgBaseUrl + '/' + scope.row.img]"
                            class="product-image" />
                    </template>
                </el-table-column>
                <el-table-column prop="num" label="剩余数量" width="100" align="center" />
                <el-table-column prop="desrc" label="商品描述" min-width="200" show-overflow-tooltip />
                <el-table-column label="操作" width="180" align="center" fixed="right">
                    <template slot-scope="scope">
                        ...
                        <el-button type="text" size="small" icon="el-icon-delete" class="delete-btn"
                            @click="handleDelete(scope.$index, scope.row)" v-if="scope.row.role !== 'SUPER_ADMIN'">删除</el-button>
                    </template>
                </el-table-column>
            </el-table>
            ...
            <!-- 新增 弹出栏 -->
            <el-dialog title="添加商品" :visible.sync="formVisibleAdd" width="500px" class="dialog-form" :modal="false">
                <el-form ref="dialogForm" :model="dialogForm" :rules="formRules" label-width="100px">
                    <div class="avatar-upload">
                        <el-upload class="avatar-uploader" :action="uploadUrl" :show-file-list="false"
                            :on-success="handleAvatarSuccess" :before-upload="beforeAvatarUpload">
                            <el-image v-if="dialogForm.img" :src="imgBaseUrl + '/' + dialogForm.img" class="avatar" />
                    ...
                    <el-form-item prop="name" label="商品名称">
                        <el-input v-model="dialogForm.name" placeholder="请输入商品名称"></el-input>
                    </el-form-item>
                    <el-form-item prop="price" label="商品价格">
                        <el-input v-model="dialogForm.price" placeholder="请输入商品价格" @input="handleInput('num')" />
                    </el-form-item>
                    <el-form-item prop="num" label="剩余数量">
                        <el-input v-model.number="dialogForm.num" placeholder="请输入商品数量" @input="handleInput('num')" />
                    </el-form-item>
                    <el-form-item prop="desrc" label="商品描述">
                        <el-input resize="false" type="textarea" :rows="4" v-model="dialogForm.desrc"
                            placeholder="请输入商品描述"></el-input>
                    </el-form-item>

                    <div class="dialog-footer">
                        <el-button @click="formVisibleAdd = false">取消</el-button>
                        <el-button type="primary" :disabled="isSubmit" @click="handleAdd('dialogForm')">确定</el-button>
                    </div>
                </el-form>
            </el-dialog>

            <!-- 编辑 弹出栏 -->
            <el-dialog title="修改商品信息" :visible.sync="formVisibleEdit" width="500px" class="dialog-form">
                <el-form ref="dialogForm" :model="dialogForm" :rules="formRules" label-width="100px">
                    <div class="avatar-upload">
                        <el-upload class="avatar-uploader" :action="uploadUrl" :show-file-list="false"
                            :on-success="handleAvatarSuccess" :before-upload="beforeAvatarUpload">
                    ...
                    <el-form-item prop="num" label="剩余数量">
                        <el-input v-model.number="dialogForm.num" min="0" placeholder="请输入商品数量" @input="handleInput('num')" />
                    </el-form-item>
                    <el-form-item prop="desrc" label="商品描述">
                        <el-input resize="false" type="textarea" :rows="4" v-model="dialogForm.desrc"
                            placeholder="请输入商品描述"></el-input>
                    ...
                    <div class="dialog-footer">
                        ...
                        <el-button type="primary" :disabled="isSubmit" @click="handleEditSave('dialogForm')">确定</el-button>
                    </div>
                </el-form>
            </el-dialog>
...
export default {
    ...
    methods: {
      ...
      if (this.dialogForm[field] < 0) {
        ...
        handleAdd(formName) {
            this.$refs[formName].validate(valid => {
                if (valid) {
                    Request.post("/goods/save", this.dialogForm).then(response => {
                        if (response.code == 0) {
                            this.$message({
                                showClose: true,
                                message: '商品添加成功',
        ...
        handleDelete(index, row) {
            this.$confirm('此操作将删除选中数据, 是否继续?', '提示', {
                ...
                Request.delete("/goods/deleteBatch?ids=" + row.id).then(response => {
                    if (response.code == 0) {
                        this.$message({
                            type: 'success',
                            message: '删除成功!'
        ...
        batchDelete() {
            if (this.multipleSelection.length < 1) {
                ...
                Request.delete(`/goods/deleteBatch?ids=${this.multipleSelection.join(',')}`).then(res => {
                    if (res.code === '0') {
                        this.$message({
                            showClose: true,
                            message: '批量删除成功',
      ...
      fetchData() {
        ...
        handleEditSave(formName) {
            this.$refs[formName].validate(valid => {
                if (valid) {
                    Request.put("/goods/update",this.dialogForm).then(response => {
                        if (response.code == 0) {
                            this.$message({
                                showClose: true,
                                message: '更新成功',
  ...
  if (this.tableData.length) {
    const params = {
      header: ['编号', '商品名称', '商品价格', '剩余数量', '商品描述'],
      key: ['id', 'name', 'price', 'num', 'desrc'],
      data: this.tableData,
      autoWidth: true,
      fileName: '商品信息表',
      bookType: type
