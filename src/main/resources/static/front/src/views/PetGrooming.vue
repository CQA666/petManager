...
export default {
    ...
    methods: {
      ...
      if (value) {
        ...
        handleModifyStatus(index, row,status) {
              ...
              data.status=status
              Request.put("/petGroomingService/update",data).then(res=>{
                  if(res.code == 0){
                      this.$message({
                          showClose: true,
                          message: '修改成功',
        ...
        handleDelete(index, row) {
            this.$confirm('此操作将删除选中数据, 是否继续?', '提示', {
                ...
                Request.delete("/petGroomingService/deleteBatch?ids=" + row.id).then(response => {
                    if (response.code == 0) {
                        this.$message({
                            type: 'success',
                            message: '删除成功!'
    ...
    handleEditSave(formName) {
        this.$refs[formName].validate(valid => {
            if (valid) {
                Request.put("/petGroomingService/update",this.dialogForm).then(response => {
                    if (response.code == 0) {
                        this.$message({
                            showClose: true,
                            message: '更新成功',
