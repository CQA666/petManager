...
export default {
       ...
       handleSelectionChange(val) {
        ...
        handleAdd(formName) {
            this.$refs[formName].validate(valid => {
                if (valid) {
                    Request.post("/trainingName/save", this.dialogForm).then(response => {
                        if (response.code == 0) {
                            this.$message({
                                showClose: true,
                                message: '训练名称添加成功',
        ...
        handleDelete(index, row) {
            this.$confirm('此操作将删除选中数据, 是否继续?', '提示', {
                ...
                Request.delete("/trainingName/deleteBatch?ids=" + row.id).then(response => {
                    if (response.code == 0) {
                        this.$message({
                            type:'success',
                            message: '删除成功!'
 ...
 batchDelete() {
        ...
        handleEditSave(formName) {
            this.$refs[formName].validate(valid => {
                if (valid) {
                    Request.put("/trainingName/update",this.dialogForm).then(response => {
                        if (response.code == 0) {
                            this.$message({
                                showClose: true,
                                message: '更新成功',
