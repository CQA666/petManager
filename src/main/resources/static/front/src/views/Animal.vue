<template>
    <div class="animal-manager">
        <el-card shadow="hover" class="main-card">
            ...
            <div class="action-bar">
                ...
                <div class="right-actions">
                    <el-button v-if="userInfo.role != 'USER'" type="primary" icon="el-icon-plus" size="medium" @click="handleCreate">添加宠物</el-button>
                    <el-button v-if="userInfo.role != 'USER'" type="success" icon="el-icon-download" size="medium" @click="exportVisible = true">导出数据</el-button>
                </div>
            </div>
            ...
            <el-form ref="searchForm" :inline="true" :model="listQuery" class="search-form">
                <el-form-item label="宠物名字">
                    <el-input v-model="listQuery.name" placeholder="请输入宠物名字" prefix-icon="el-icon-search" />
                </el-form-item>
                <el-form-item>
            ...
            <el-table ref="multipleTable" v-loading="listLoading" :data="tableData" @selection-change="handleSelectionChange"
                class="animal-table" border stripe>
                <el-table-column type="selection" width="55" align="center" />
                <el-table-column prop="img" label="宠物头像" width="100" align="center">
                    <template #default="scope">
                        <el-image :src="imgBaseUrl + '/' + scope.row.img" alt="宠物头像"
                ...
                <el-table-column prop="sex" label="宠物性别" width="80" align="center" />
                <el-table-column prop="age" label="宠物年龄" width="100" align="center">
                    <template #default="scope">
                        {{ scope.row.age }}岁
                    </template>
                </el-table-column>
                <el-table-column prop="status" label="宠物状态" width="100" align="center">
                    <template #default="scope">
                        <el-tag :type="scope.row.status === '待领养' ? 'warning' : 'success'" size="small">
                            {{ scope.row.status }}
                        </el-tag>
                    </template>
                </el-table-column>
                <el-table-column prop="breed" label="宠物品种" min-width="120" />
                <el-table-column prop="registrationDate" label="注册日期" width="120" align="center">
                    <template #default="scope">
                        {{ scope.row.registrationDate }}
                    </template>
                </el-table-column>
                <el-table-column prop="behaviorTraits" label="行为特征" min-width="120" />
                <el-table-column prop="sterilizationStatus" label="绝育状况" width="100" align="center" />
                <el-table-column prop="color" label="颜色" width="100" align="center" />
                <el-table-column prop="trainingLevel" label="训练状况" width="100" align="center" />
                <el-table-column prop="specialNeeds" label="特殊要求" min-width="120" />
                <el-table-column prop="descr" label="宠物介绍" min-width="200" show-overflow-tooltip />
                <el-table-column label="操作" width="180" align="center" fixed="right" v-if="userInfo.role != 'USER'">
                    <template #default="scope">
        ...
        <!-- 新增/编辑对话框 -->
        <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="500px" class="animal-dialog" :close-on-click-modal="false"   :modal="false"  >
            <el-form ref="dialogForm" :model="dialogForm" :rules="formRules" label-width="100px" class="dialog-form">
                <div class="avatar-upload">
                    <el-upload class="avatar-uploader" :action="uploadUrl" :show-file-list="false"
                        :on-success="handleAvatarSuccess" :before-upload="beforeAvatarUpload">
                        <img v-if="dialogForm.img" :src="imgBaseUrl + '/' + dialogForm.img" class="avatar" />
                ...
                <el-form-item prop="breed" label="宠物品种">
                    <el-select v-model="dialogForm.breed" placeholder="请选择品种" style="width: 100%">
                        <el-option v-for="breed in breedList" :key="breed.breedId" :label="breed.breedName"
                            :value="breed.breedName">
                        </el-option>
                    </el-select>
                </el-form-item>
                <el-form-item prop="descr" label="宠物介绍">
                    <el-input type="textarea" :rows="4" v-model="dialogForm.descr" placeholder="请输入宠物介绍" />
                </el-form-item>
                <el-form-item prop="registrationDate" label="注册日期">
                    <el-date-picker v-model="dialogForm.registrationDate" type="date" placeholder="选择日期" style="width: 100%">
                    </el-date-picker>
                </el-form-item>
                <el-form-item prop="behaviorTraits" label="行为特征">
                    <el-input v-model="dialogForm.behaviorTraits" placeholder="请输入行为特征" />
                </el-form-item>
                <el-form-item prop="sterilizationStatus" label="绝育状况">
                    <el-select v-model="dialogForm.sterilizationStatus" placeholder="请选择绝育状况" style="width: 100%">
                ...
                <el-form-item prop="trainingLevel" label="训练状况">
                    <el-select v-model="dialogForm.trainingLevel" placeholder="请选择训练状况" style="width: 100%">
                ...
                <el-form-item prop="specialNeeds" label="特殊要求">
                    <el-input v-model="dialogForm.specialNeeds" placeholder="请输入特殊要求" />
                </el-form-item>
            </el-form>
            <template #footer>
                <div class="dialog-footer">
                    <el-button @click="dialogVisible = false">取 消</el-button>
                    <el-button type="primary" :disabled="isSubmit" @click="isEdit ? handleEditSave('dialogForm') : handleAdd('dialogForm')">确 定</el-button>
                </div>
            </template>
        </el-dialog>
        ...
        return {
            ...
            formRules: {
                name: [
                    ...
                    { min: 1, max: 255, message: '宠物名字长度必须在1到255个字符之间', trigger: 'blur' }
                ],
                sex: [
                ...
                type: [
                    ...
                    { min: 1, max: 255, message: '宠物种类长度必须在1到255个字符之间', trigger: 'blur' }
                ],
                breed: [
                ...
                descr: [
                    ...
                    { min: 1, max: 255, message: '宠物介绍长度必须在1到255个字符之间', trigger: 'blur' }
                ],
                registrationDate: [
    ...
    methods: {
        ...
        handleAdd(formName) {
            this.$refs[formName].validate(valid => {
                if (valid) {
                    this.isSubmit = true;
                    Request.post("/animal/save", this.dialogForm).then(response => {
                        if (response.code == 0) {
                            this.$message({
                                showClose: true,
                                message: '添加成功',
        ...
        handleDelete(index, row) {
            this.$confirm('此操作将删除选中数据, 是否继续?', '提示', {
                ...
                Request.delete(`/animal/deleteBatch?ids=${row.id}` ).then(response => {
                    if (response.code == 0) {
                        this.$message({
                            type: 'success',
                            message: '删除成功!'
        ...
        batchDelete() {
            if (this.multipleSelection.length < 1) {
                ...
                Request.delete(`/animal/deleteBatch?ids=${this.multipleSelection.join(',')}`).then(res => {
                    if (res.code === '0') {
                        this.$message({
                            showClose: true,
                            message: '批量删除成功',
        ...
        fetchData(pagination) {
            // 接收 Pagination 组件发出的事件载荷，同步更新 listQuery
            if (pagination) {
                this.listQuery.currentPage = pagination.page
        ...
        handleEditSave(formName) {
            this.$refs[formName].validate(valid => {
                if (valid) {
                    this.isSubmit = true;
                    Request.put("/animal/update/" + this.dialogForm.id, this.dialogForm).then(response => {
                        if (response.code == 0) {
                            this.$message({
                                showClose: true,
                                message: '更新成功',
        ...
        exportTable(type) {
            if (this.tableData.length) {
                const params = {
                    header: ['宠物名字', '宠物种类', '宠物性别', '宠物年龄', '宠物状态', '宠物品种', '宠物介绍'],
                    key: ['name', 'type', 'sex', 'age', 'status', 'breed', 'descr'],
